  ...
  describe "dashboard" do
    ...
    describe "listing page" do
      it_behaves_like "creator dashboard page", "Analytics" do
        let(:path) { utm_links_dashboard_index_path }
      end

      it "shows the empty state when there are no UTM links" do
        visit utm_links_dashboard_index_path
      ...
      it "paginates the UTM links" do
        ...
        expect(page).to have_current_path(utm_links_dashboard_index_path({ page: 2 }))
      end
      ...
      it "sorts UTM links by different columns and direction and allows pagination" do
        ...
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=link&direction=asc")
        expect(page).to have_table_row({ "Link" => "A Link" })
        expect(page).to_not have_table_row({ "Link" => "B Link" })
        click_on "Next"
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=link&direction=asc&page=2")
        expect(page).to have_table_row({ "Link" => "B Link" })
        expect(page).to_not have_table_row({ "Link" => "A Link" })

        # Sort by "Source" column
        find_and_click("th", text: "Source")
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=source&direction=asc")
        expect(page).to have_table_row({ "Link" => "B Link", "Source" => "newsletter" })
        expect(page).to_not have_table_row({ "Link" => "A Link", "Source" => "twitter" })
        click_on "Next"
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=source&direction=asc&page=2")
        ...
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=medium&direction=asc")
        expect(page).to have_table_row({ "Link" => "B Link", "Medium" => "email" })
        expect(page).to_not have_table_row({ "Link" => "A Link", "Medium" => "social" })
        click_on "Next"
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=medium&direction=asc&page=2")
        ...
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=campaign&direction=asc")
        expect(page).to have_table_row({ "Link" => "A Link", "Campaign" => "spring" })
        expect(page).to_not have_table_row({ "Link" => "B Link", "Campaign" => "winter" })
        click_on "Next"
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=campaign&direction=asc&page=2")
        ...
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=clicks&direction=asc")
        expect(page).to have_table_row({ "Link" => "B Link", "Clicks" => "1" })
        expect(page).to_not have_table_row({ "Link" => "A Link", "Clicks" => "3" })
        click_on "Next"
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=clicks&direction=asc&page=2")
        ...
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=revenue_cents&direction=asc")
        expect(page).to have_table_row({ "Link" => "A Link", "Revenue" => "$15" })
        expect(page).to_not have_table_row({ "Link" => "B Link", "Revenue" => "$20" })
        click_on "Next"
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=revenue_cents&direction=asc&page=2")
        ...
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=conversion_rate&direction=asc")
        expect(page).to have_table_row({ "Link" => "A Link", "Conversion" => "66.67%" })
        expect(page).to_not have_table_row({ "Link" => "B Link", "Conversion" => "100%" })
        click_on "Next"
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=conversion_rate&direction=asc&page=2")
      ...
      it "filters UTM links by search query by adhering to the current column sort order" do
        ...
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=link&direction=asc")
        find_and_click("th", text: "Link")
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=link&direction=desc")
        ...
        expect(page).to have_table_row({ "Link" => utm_link1.title })
        expect(page).to_not have_table_row({ "Link" => utm_link2.title })
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=link&direction=desc&page=2")
        ...
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=link&direction=desc&query=+Sale+++++")
        ...
        expect(page).to_not have_button("Next")
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}?key=link&direction=desc&query=TwiTTer")

        # Shows filtered results on accessing the page with the 'query' param
        visit "#{utm_links_dashboard_index_path}?key=source&direction=asc&query=PROMO"
        expect(page).to have_table_row({ "Link" => utm_link2.title })
        expect(page).not_to have_table_row({ "Link" => utm_link1.title })
        expect(page).to_not have_button("Next")
    ...
    describe "create page" do
      ...
      it "renders the create link form" do
        ...
        visit "#{utm_links_dashboard_index_path}/new"

        expect(page).to have_text("Create link")
        expect(page).to have_link("Cancel", href: utm_links_dashboard_index_path)
      ...
      it "shows validation errors" do
        ...
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}/new")
      end
      ...
      it "creates a UTM link" do
        ...
        expect(page).to have_current_path(utm_links_dashboard_index_path)
    ...
    describe "duplicate link" do
      it "pre-fills the create page with the existing UTM link's values" do
        ...
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}/new?copy_from=#{existing_utm_link.external_id}")
    ...
    describe "update page" do
      it "performs validations and updates the UTM link" do
        ...
        expect(page).to have_current_path("#{utm_links_dashboard_index_path}/#{utm_link.external_id}/edit")
