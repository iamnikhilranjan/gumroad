...
describe("Product panel on creator profile - Sort/Filter", type: :system, js: true) do
  before do
    ...
    @preview_image_url = "#{S3_BASE_URL}/specs/kFDzu.png"
  ...
  describe "Filetype filter" do
    before do
      ...
      # seed ZIP
      create(:product_file, link: @b, url: "#{S3_BASE_URL}/specs/preorder.zip")
      @c.product_files << create(:product_file, url: "#{S3_BASE_URL}/specs/preorder.zip")

      # seed MP3
      @c.product_files << create(:product_file, url: "#{S3_BASE_URL}/specs/magic.mp3")
      @c.save!
