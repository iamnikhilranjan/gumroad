...
class CreatorAnalytics::Sales
  ...
  def initialize(user:, products:, dates:)
    ...
    @products = products
    @dates = dates
    @timezone_offset = @user.timezone_formatted_offset(at: @dates.last)
    @query = PurchaseSearchService.new(SEARCH_OPTIONS).body[:query]
    @query[:bool][:filter] << { terms: { product_id: @products.map(&:id) } }
    @query[:bool][:must] << { range: { created_at: { time_zone: @timezone_offset, gte: @dates.first.to_s, lte: @dates.last.to_s } } }
  end

  def by_product_and_date
    sources = [
      { product_id: { terms: { field: "product_id" } } },
      { date: { date_histogram: { time_zone: @timezone_offset, field: "created_at", calendar_interval: "day", format: "yyyy-MM-dd" } } }
    ]
