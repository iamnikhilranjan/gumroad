...
class CreatorAnalytics::Following
  ...
  def net_total(before_date: nil)
    must = [{ range: { timestamp: { time_zone: user.timezone_formatted_offset(at: before_date), lt: before_date.to_s } } }] if before_date
      ...
      body = {
        query: {
          bool: {
            ...
            must: [{ range: { timestamp: { time_zone: user.timezone_formatted_offset(at: end_date), gte: start_date.to_s, lte: end_date.to_s } } }]
          }
        },
        aggs: {
          dates: {
            date_histogram: { time_zone: user.timezone_formatted_offset(at: end_date), field: "timestamp", calendar_interval: "day" },
            aggs: names_aggs
          }
        },
        size: 0
